...
let DataProvider = ({ children }) => {
  ...
  let { updateCartedProductToServer, updateCartedProductToClient } =
    UseUpdate();
  ...
  useEffect(() => {
    ...
    if (
      ...
      return setAuth({ user: "", tokenx: "" });
    } else {
      console.log(
        " if (localStorage.getItem('tokenx'))",
        localStorage.getItem("userx")
      );
      return setAuth({
        user: JSON.parse(localStorage.getItem("userx")),
